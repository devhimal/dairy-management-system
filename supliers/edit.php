...
if (isset($_POST['f_no'])) {
    ...
    if ($validation['valid'] == TRUE) {
        $sql = "UPDATE supliers SET  f_no =  '{$_POST['f_no']}' ,f_id =  '{$_POST['f_id']}' ,  f_name =  '{$_POST['f_name']}' , f_locallity =  '{$_POST['f_locallity']}' ,  f_ac =  '{$_POST['f_ac']}' ,  f_phone =  '{$_POST['f_phone']}'   WHERE f_no = '$f_no' ";
...
$suplier_to_edit = mysqli_query($conn, "SELECT * FROM supliers WHERE f_no =" .  stripslashes($f_no));

$row = mysqli_fetch_array($suplier_to_edit);
//echo $validation['nulls'];
?>
